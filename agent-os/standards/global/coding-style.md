...
- **NEVER use `.forEach()`**: Use `for...of` for side effects, `.map()/.filter()/.reduce()` for transformations
- **NEVER build objects with loop mutation**: Don't create empty objects and populate with loops. Use `Object.fromEntries()` + `.map()` + `.filter()`
- **NEVER conditionally add keys one-by-one**: Don't write `if (x) obj.x = x`. Collect all values first, then filter out undefined
- **PREFER transformation pipelines**: Chain `.map()`, `.filter()`, `.sort()` in a single expression over intermediate mutable variables

```typescript
// Bad: forEach + mutation
const result: Config = {}
keys.forEach((key) => {
...
const result = Object.fromEntries(
  keys
    .map((key) => [key, getValue(key)] as const)
    .filter(([_, v]) => v !== undefined)
) as Config
```

See `effect/iteration.md` for detailed patterns and examples.
